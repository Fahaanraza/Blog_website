...
# load the current user for the authentication access
@login_manager.user_loader
def load_user(user_id):
    return Users.query.get(int(user_id))

# admin only rights initialization for the pages creation
def admin_only(f):
    @wraps(f)
...
class Users(UserMixin,db.Model):
    ...
    coments=db.relationship('Comments',back_populates='comment_author')
# db.create_all()
...
class Comments(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    author_id = db.Column(db.Integer, db.ForeignKey("users.id"))
    comment_author = db.relationship("Users", back_populates="coments")
...
def register():
    ...
    if form.validate_on_submit():
        ...
        new_data = Users(
            name=request.form.get('name'), email=request.form.get('email'), password=hash_pass)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        password = request.form.get('password')
        if user == None:
            flash('User does not Exist!!Register now')
...
def show_post(post_id):
    ...
    return render_template("post.html", post=requested_post,current_user=current_user,form=form,all_comments=all_comments,logged_in=current_user.is_authenticated)
